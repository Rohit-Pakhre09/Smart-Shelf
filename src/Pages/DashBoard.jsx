...
import { fetchBooks, fetchIssuedBooks } from "../modules/booksSlice";
...
const DashBoard = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        console.error("Error fetching books or issued books:", err);
      }
    };
    fetchData();
  }, [dispatch]);
  ...
  const topBooks = [...books]
    ...
    .sort((a, b) => b.popularity - a.popularity) // Sort descending
    .slice(0, 5);
  ...
  const issuedBooksCount = loading ? (
    <span className="inline-flex space-x-1">
      <span className="w-2 h-2 bg-indigo-300 rounded-full animate-bounce"></span>
      <span className="w-2 h-2 bg-indigo-300 rounded-full animate-bounce [animation-delay:0.2s]"></span>
      <span className="w-2 h-2 bg-indigo-300 rounded-full animate-bounce [animation-delay:0.4s]"></span>
  ...
  const availableBooks = loading ? (
    ...
    books.length - issuedBooks.length
  );
  ...
  const inactiveMembers = members.length && typeof activeMembers === "number" ? members.length - activeMembers : 0;
  ...
  const topBook = loading ? (
    ...
    [...books].sort((a, b) => (b.popularity || 0) - (a.popularity || 0))[0]?.title || "N/A"
  );
    ...
    <section className="flex min-h-screen">
      ...
      <div className="flex flex-col flex-1 transition-all duration-500">
        ...
        <section className="flex-1 pt-0 lg:pt-[70px] m-0 lg:m-2.5 transition-all duration-500 ease-in-out">
          <div
            className={`h-[87vh] overflow-y-scroll scrollbar-thin overflow-x-hidden pr-0 lg:pr-2 rounded-xl transition-all duration-500 lg:mt-6 ${open ? "lg:ml-68 lg:w-[calc(100%-17rem)]" : "lg:ml-24 lg:w-[calc(100%-6rem)]"
              }`}
          >
            {/* Dashboard Heading */}
            <p
              className={`${lightTheme ? "text-white" : "text-black"} text-3xl pb-3 mt-5 pl-5 font-bold animation transition-all duration-500`}
            >
              Dashboard
            </p>
            ...
            {(error || membersError) && (
              <div className="mx-5 mb-4 p-3 bg-red-100 text-red-800 rounded-lg text-sm">
                {error && <div>{error}</div>}
                {membersError && <div>{membersError}</div>}
              </div>
            )}
            ...
            <div className="min-h-full flex flex-col gap-5 p-3">
              <section className="flex flex-col items-center lg:flex-row gap-5 lg:items-start lg:justify-between">
                ...
                <div className="flex flex-col gap-5 w-full lg:flex-1">
                  {/* Quick Stats */}
                  <div
                    className={`w-auto ${lightTheme ? "bg-slate-900 text-white" : "bg-white"} rounded-xl p-6 shadow-lg animation flex flex-col gap-6`}
                  >
                    <h2 className="text-xl md:text-3xl font-bold mb-2 pb-2 flex items-center gap-2">
                      <svg
                        ...
                        viewBox="0 0 24 24"
                        fill="currentColor"
                        className={`size-8 animation ${lightTheme ? "text-blue-500" : "text-indigo-500"}`}
                      >
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-5 text-center">
                      {/* Total Books */}
                      <div
                        className={`p-5 rounded-xl shadow-blue-300 ${lightTheme ? "bg-slate-800 shadow-md border-slate-700" : "bg-gray-100 shadow-sm border-gray-100"
                      ...
                      >
                        <div className="flex items-center justify-center gap-2">
                          <svg
                            ...
                            stroke="currentColor"
                          >
                            <path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M12 6v12m8-6H4" />
                          </svg>
                          <p className="text-3xl font-bold">{totalBooks}</p>
                        </div>
                        <p className="text-sm opacity-80 font-medium md:text-md">Total Books</p>
                        <svg viewBox="0 0 100 40" className="w-full h-14">
                          <rect x="10" y="20" width="20" height="12" fill="#3b82f6" rx="2" />
                          <rect x="35" y="15" width="20" height="12" fill="#2563eb" rx="2" />
                          <rect x="60" y="10" width="20" height="12" fill="#1d4ed8" rx="2" />
                        </svg>
                      </div>

                      {/* Issued Books */}
                      <div
                        className={`p-5 rounded-xl shadow-red-300 ${lightTheme ? "bg-slate-800 shadow-md border-slate-700" : "bg-gray-100 shadow-sm border-gray-100"
                          } border flex flex-col justify-between hover:scale-[1.03] animation gap-3`}
                      >
                        <div className="flex items-center justify-center gap-2">
                          <svg
                        ...
                        <p className="text-sm opacity-80 font-medium md:text-md">Issued Books</p>
                        <svg viewBox="0 0 100 40" className="w-full h-14">
                          <circle cx="20" cy="25" r="6" fill="#ef4444" />
                          <circle cx="50" cy="15" r="6" fill="#ef4444" />
                          <circle cx="80" cy="30" r="6" fill="#ef4444" />
                          <path d="M20 25 L50 15 L80 30" stroke="#ef4444" strokeWidth="2" fill="none" />
                        </svg>
                      </div>

                      {/* Available Books */}
                      <div
                        className={`p-5 rounded-xl shadow-green-300 ${lightTheme ? "bg-slate-800 shadow-md border-slate-700" : "bg-gray-100 shadow-sm border-gray-100"
                          } border flex flex-col justify-between hover:scale-[1.03] animation gap-3`}
                      >
                        <div className="flex items-center justify-center gap-2">
                          <svg
                        ...
                        <p className="text-sm opacity-80 font-medium md:text-md">Available Books</p>
                        <svg viewBox="0 0 36 36" className="w-14 h-14 mx-auto">
                          ...
                          <path
                            ...
                            strokeLinecap="round"
                            fill="none"
                            strokeDasharray={`${books.length && issuedBooks.length && typeof availableBooks === "number" && typeof totalBooks === "number"
                  ...
                  {/* Additional Info */}
                  <div
                    className={`h-auto lg:h-70 w-full ${lightTheme ? "bg-slate-900 text-white" : "bg-white"} rounded-lg p-5 shadow-md animation flex flex-col gap-3 duration-500 transition-all`}
                  >
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-2 gap-3">
                      <div
                        className={`p-3 rounded-lg shadow border ${lightTheme ? "bg-slate-800 border-slate-700" : "bg-gray-100 border-gray-200"
                          } animation`}
                      >
                ...
                <div className="flex flex-col gap-5 w-full lg:w-1/3">
                  ...
                  >
                    {topBooks.length > 0 ? (
                      ...
                      <p className={`${lightTheme ? "text-neutral-400" : "text-black"} italic flex justify-center items-center`}>
                        <img src={growingChart} alt="Loading Bar Chart" className="w-60 h-60" />
                      </p>
                    )}
                  </div>
                  <div
                    className={`h-70 flex justify-center items-center ${lightTheme ? "bg-slate-900 text-white" : "bg-white"} rounded-lg p-5 shadow-md animation`}
                  >
                    {books.length > 0 ? (
                      <Pie data={conditionData} options={conditionOptions} />
                    ) : (
                      <p className={`${lightTheme ? "text-neutral-400" : "text-black"} italic flex justify-center items-center`}>
                        <img src={pieChart} alt="Loading Pie Chart" className="w-40 h-40" />
                      </p>
                    )}
                  </div>
                </div>
              ...
              <section
                className={`h-auto w-full mb-3 ${lightTheme ? "bg-slate-900 text-white" : "bg-white"} rounded-lg p-5 shadow-md animation`}
              >
                <div className="flex items-center justify-between">
                  ...
                  <Link
                    to="/books"
                    className={`px-2 py-1 md:px-3 md:py-2 ${lightTheme ? "bg-blue-500 text-white hover:bg-blue-700" : "bg-indigo-500 hover:bg-indigo-700 text-white"
                  ...
                  {books && books.length > 0 ? (
                    books.slice(0, 9).map((book, index) => (
                      <div key={index} className="flex flex-col items-center">
                        ...
                        <p className="mt-5 text-xs sm:text-sm font-medium text-center truncate w-full" title={book.title}>
                          {book.title}
                        </p>
                      </div>
                    ))
                    ...
                    <div className="flex items-center justify-center">
                      <img src={booksFallback} alt="Loading Books" className="w-40 h-40" />
                    </div>
                  )}
                </div>
              </section>
                ...
                <ul className="space-y-3">
                  {members.length > 0 ? (
                    members.slice(0, 5).map((member, i) => (
                      <li
                        key={i}
                        className={`p-4 rounded-xl shadow-sm flex justify-between items-center border animation ${lightTheme ? "bg-slate-800 border-slate-700 hover:bg-slate-700" : "bg-gray-50 border-gray-200 hover:bg-gray-100"
                          }`}
                      >
                        <div className="flex items-center gap-3">
                          ...
                          <div>
                            <span className="font-medium text-base md:text-lg block truncate max-w-[200px]">{member.name}</span>
                            <span className="text-xs text-gray-400">{member.email}</span>
                          </div>
                        </div>
                        <span
                          className={`px-3 w-20 text-center py-2 rounded-full text-xs font-semibold tracking-wide ${member.status === "active" ? "bg-green-100 text-green-700" : "bg-red-100 text-red-700"
                            } capitalize`}
                        >
                          {member.status}
                        </span>
                    ...
                    <div className="flex items-center justify-center">
                      <img src={membersFallback} alt="No members available" className="w-40 h-40" />
                    </div>
                  )}
                </ul>
              </section>
